  ...
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      try {
        ...
        // Validation
        if (!tradeDateStr || !stockCode || !companyName || !typeStr || !quantity || !price) {
          results.failed++;
        ...
        // Validate type
        if (typeStr !== "BUY" && typeStr !== "MUA" && typeStr !== "SELL" && typeStr !== "BÁN") {
          results.failed++;
          results.errors.push(`Dòng ${i + 2}: Loại lệnh không hợp lệ (phải là BUY/MUA hoặc SELL/BÁN)`);
          continue;
        }
